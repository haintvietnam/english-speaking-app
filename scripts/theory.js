// Theory page specific functionality
...
function initSoundPlayback() {
  ...
  playButtons.forEach(button => {
    button.addEventListener('click', function () {
      const soundId = this.getAttribute('onclick')?.match(/playSound\('(.+?)'\)/)?.[1];
      if (soundId) {
        playSound(soundId);
      }
    });
...
function initStressExercise() {
  ...
  stressWords.forEach(word => {
    word.addEventListener('click', function () {
      ...
      if (clickedSyllable) {
        const syllableIndex = Array.from(syllables).indexOf(clickedSyllable) + 1;
        ...
        if (syllableIndex === correctStress) {
          ...
          showNotification('Try again! The stress is on a different syllable.', 'error');
...
function updateProgressIndicator() {
  ...
  if (indicator && progressText && progressFill) {
    ...
    if (percentage === 100) {
      showNotification('Congratulations! You\'ve completed all theory sections! 🎉', 'success');
      indicator.style.background = 'linear-gradient(135deg, #38b2ac 0%, #319795 100%)';
